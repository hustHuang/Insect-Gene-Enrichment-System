...
var IIPMooViewer = new Class({
   ...
   */
    positionZone: function(){
        ...
        //$j('.canvas').imgAreaSelect({handles: true, fadeSpeed: 200});
	this.zoomEvent();
        var IAS=$j('.canvas').imgAreaSelect({   //右键选择部分区域来进行更详细的显示
            ...
            onSelectEnd:function(img, getSelection){
                isMouseDown=false;
                //var input=window.parent.document.getElementById('it');
                //$j(input).val('');
                ...
                //换算成实际起始坐标
                var x=Math.round(radio*parseInt(getSelection.x1));                  
...
//                '<input id="reCluster" type="button" value="Recluster"><br/>'+
    ...
    zoomEvent:function(){
        var initialWidth=3844;
        var initialHeight=3544;         //原图片的大小
