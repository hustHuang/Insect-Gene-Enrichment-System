...
var IIPMooViewer = new Class({
   ...
   */
    createWindows: function(){
        ...
        this.touch = new Drag( this.canvas, {
            ...
            onDrag: this.positionTags.bind(this) //added by huang
        });
   ...
   */
    ...
    positionZone: function(){
        ...
        var IAS=$j('.canvas').imgAreaSelect({   //右键选择部分区域来进行更详细的显示
            ...
            onSelectEnd:function(img, getSelection){
                ...
                var initialWidth=ds[datatype].w;  //原图片的大小
                var initialHeight=ds[datatype].h;
                var currentWidth=parseInt($j('.canvas').css('width'));
                //var currentHeight=parseInt($j('.canvas').css('height'));
                var row_num=ds[datatype].row_num;       //基因的横列数目
                var col_num=ds[datatype].col_num;           
                ...
                var rowStartIndex=Math.round((y-(initialHeight-2*row_num))/2);
                var rowLength=Math.round(h/2);
                $j('#matrixBtn #r1').val(rowStartIndex);
                $j('#matrixBtn #r2').val(rowStartIndex+rowLength);
                $j('#matrixBtn #c1').val(colStartIndex);
    ...
    positionTags:function(){          //对Tags进行定位
        $j('.tag').show();
        var initialWidth=ds[datatype].w;
        var initialHeight=ds[datatype].h;         //原图片的大小
        var row_num=ds[datatype].row_num;
        var col_num=ds[datatype].col_num;               //基因的横列数目
        ...
        //横纵线条
        var x_length=$j('.x_line').length;
        for(var m=0;m<x_length;m++){
            var lineX=$j(".x_line:eq("+m+")");
            var index_X=parseInt(lineX.attr('id').split('_')[2]);
            var x_left=parseFloat((index_X/col_num)*width+x);
            lineX.css('height',height);
            lineX.css('left',x_left);
            lineX.css('top',y);
        }
        var y_length=$j('.y_line').length;
        for(var n=0;n<y_length;n++){
            var lineY=$j(".y_line:eq("+n+")");
            var index_Y=parseInt(lineY.attr('id').split('_')[2]);
            var y_top=parseInt((index_Y/row_num)*height+(initialHeight-2*row_num)*radio+y);
            lineY.css('width',width);
            lineY.css('left',x);
            lineY.css('top',y_top);
        }
        //对应的各基因点
        ...
        for(var i=0;i<l;i++){
            var tag=$j('.tag').eq(i);
            var pos=tag.attr('pos');//alert(pos);
            var x_pos=parseInt(pos.split('_')[0]);
            var y_pos=parseInt(pos.split('_')[1]);
            var top=parseFloat((parseInt(y_pos)/row_num)*height+y)+parseFloat((initialHeight-2*row_num)*radio);
            var left=parseFloat((parseInt(x_pos)/col_num)*width+x);
