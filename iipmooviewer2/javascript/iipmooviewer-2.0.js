...
var IIPMooViewer = new Class({
   ...
   */
    initialize: function( main_id, options ) {
        ...
        this.prefix = options.prefix || 'iipmooviewer2/images/';
   ...
   */
    showPopUp: function( text ) {
        ...
        var del;
        if(Browser.ie&&Browser.version<9) del = function(){
            fs.destroy();
        };
        else del = function(){
   ...
   */
    createWindows: function(){
        ...
        new Element( 'div', {
            ...
            'html': '<div><div><h2><a href="http://www.geneticinteraction.org"><img src="'+this.prefix+'32_yeast_SGA2.ico"/></a>GeneticInteractionViewer</h2><ul><li>To navigate within image: drag image within main window or drag zone within the navigation window or click an area within navigation window</li><li>To zoom in: double click with the mouse or use the mouse scroll wheel or simply press the "+" key</li><li>To zoom out: shift double click with the mouse or use the mouse wheel or press the "-" key</li><li>To rotate image clockwise: press the "r" key, anti-clockwise: press shift and "r"</li><li>To review in details, press mouse right key and drag to select specical area of interest ,and then select "Detail" in poped-up menu <li>To recluster,press mouse right key to select special area of interest, and then select "Recluster" in poped-up menu</li><li>To show/hide navigation window: press "h" key</li></ul><br/>For more information visit Tutorial : <a href="http://www.geneticinteraction.org">http://www.geneticinteraction.org</a></div></div>'
             //'html':'<div><div><h2><a href="#"><img src="'+this.prefix+'32_yeast_SGA2.ico"/></a>Heatmap View</h2>Cluster genetic interaction profiles<br/><ul><li>“Cluster genetic interaction profiles” will perform hierarchical clustering analysis on our whole genome genetic interaction data, given user submission of list of genes of interests and visualize result in heatmap style.</li></ul><br/>For more information visit <a href="#">http://www.defaultSiteUrl.edu.cn</a></div></div>'
        }).inject( container );
        ...
        var info = new Element( 'img', {
            'src': this.prefix+'32_yeast_SGA2.ico',
            'class': 'logo',
   ...
   */
    createNavigationWindow: function() {
        ...
        //alert('make navwin success!');
        $j.each(_posArray,function(i,e){
            var navTag='<div id="navTag_'+parseInt(i+1)+'" class="navTag"></div>';
            $j('.navwin').append(navTag);
            var size_w=$j('.navwin').width();
            var size_h=$j('.navwin').height();
            var rows_num=3652;
            var cols_num=3841;       //基因的横列数目
            $j('#navTag_'+parseInt(i+1)).css('left',parseInt(e.x/cols_num*size_w));
            $j('#navTag_'+parseInt(i+1)).css('top',parseInt(e.y/rows_num*size_h));                      
        });
   ...
   */
    ...
    positionZone: function(){
        ...
        //alert($j('.canvas').attr('r'));
        
        this.positionTags();
        this.getZoomRadio();        
        var IAS=$j('.canvas').imgAreaSelect({   //右键选择部分区域来进行更详细的显示
            handles: true, 
            instance: true,//设置为可实例化
            fadeSpeed: 200,
            maxHeight:parseInt(1000*$j('.canvas').attr('r')),
            maxWidth:parseInt(1000*$j('.canvas').attr('r')),
            ...
            onSelectEnd:function(img, getSelection){
                isMouseDown=false;
                //alert('x1 : '+getSelection.x1+' y1 : '+getSelection.y1+' x2 :'+getSelection.x2+' y2 :'+getSelection.y2+'<br/> w: '+getSelection.width+' h: '+getSelection.height);
                var initialWidth=7817;  //原图片的大小
                var initialHeight=7425;
                var currentWidth=parseInt($j('.canvas').css('width'));
                //var currentHeight=parseInt($j('.canvas').css('height'));
                ...
                var yLength=y-(initialHeight-2*row_num);
                if(yLength%2==1){       //纵坐标设定为起始值加偶数
                    y=y-1;
                }                           
                //换算成实际高度
                ...
                _selectedPosArray=[];
                $j.each(_posArray,function(i,e){
                    var posX=2*parseInt(e.x);
                    var posY=2*parseInt(e.y)+initialHeight-2*row_num;
                    if(posX>x&&posX<x+w&&posY>y&&posY<y+h){
                        var ee={
                            x:posX-x,
                            y:posY-y
                        };        //x,y此时为存放Tags相对截取位置的偏移
                        //ee.x=posX-x;
                        //ee.y=posY-y;                       
                        _selectedPosArray.push(ee);        //把选中的Tags信息存在全局变量中
                    }
                });          
                
                var d=x+'_'+y+'_'+w+'_'+h;
                //alert(x + ' '+ y +' '+ w + ' '+h);
                $j('.canvas').attr('d',d);  //把当前的实际截取坐标储存在d属性里面
                $j('.imgareaselect-outer:eq(2) .toolbar').remove();
                $j('.imgareaselect-outer:eq(2) form').remove();
                var btns='<div class="toolbar" style="opacity:1;z-index:5">'+
                '<form id="matrixBtn" method="GET" action="./heatmap/testdesc.R" target= "_blank">'+
                ...
                '</form></div>';
                $j('.imgareaselect-outer:eq(2)').append(btns);
                $j('.imgareaselect-outer:eq(2) .toolbar').css({
                    position:"absolute",
                    left:0,
                    top:$j('.imgareaselect-outer:eq(1)').height()+$j('.imgareaselect-selection').height()-$j('.imgareaselect-outer:eq(2) .toolbar').height()-2
                });
                $j('.imgareaselect-outer:eq(3)').css('overflow','visible');
                $j('.imgareaselect-outer:eq(3)').css('z-index','5');
                //get the gene row and col index
                var colStartIndex=Math.round(x/2)-2;
                var colLength=Math.round(w/2);
                var rowStartIndex=Math.round((y-(initialHeight-2*row_num))/2)-2;
                var rowLength=Math.round(h/2);
                $j('#matrixBtn #r1').val(rowStartIndex+1);
                $j('#matrixBtn #r2').val(rowStartIndex+rowLength);
                $j('#matrixBtn #c1').val(colStartIndex+1);
                $j('#matrixBtn #c2').val(colStartIndex+colLength);
            }
        });
        var time=parseInt(1/$j('.canvas').attr('r'));
        ...
        $j('#cancelBtn').live('click',function(){            
            $j('.tag').show();
            $j('.selected_area').show();            
            IAS.cancelSelection();      //取消选择区域
        });
    },
    
    positionTags:function(){          //对Tags进行定位
        $j('.tag').show();
        var initialWidth=7817;
        var initialHeight=7425;         //原图片的大小
        var row_num=3652;
        var col_num=3841;               //基因的横列数目
        var y=parseInt($j('.canvas').css('top')); 
        var x=parseInt($j('.canvas').css('left'));       //图片相对于窗口的偏移
        var width=parseInt($j('.canvas').css('width'));    
        var height=parseInt($j('.canvas').css('height'));//图片当前的尺寸
        var radio=width/initialWidth;
        $j('.canvas').attr('r',parseFloat(radio));       //把缩放比例存放在r属性里面
        var posArray=_posArray;             //记录位置的数组
        if($j('#x_line').length){
            var x_left=parseFloat((parseInt(posArray.x)/col_num)*width+x);
            $j('#x_line').css('height',height);
            $j('#x_line').css('left',x_left);
            $j('#x_line').css('top',y);           
        }
        if($j('#y_line').length){
            var y_top=(parseInt(posArray.y)/row_num)*height+(initialHeight-2*row_num)*radio+y;
            $j('#y_line').css('width',width);
            $j('#y_line').css('left',x);
            $j('#y_line').css('top',y_top);           
        }
        var l=$j('.tag').length;
        for(var i=0;i<l;i++){
            var tag=$j('#tag_'+parseInt(i+1));
            var pos=posArray[i];            //alert(pos);
            var top=parseFloat((parseInt(pos.y)/row_num)*height+y)+parseFloat((initialHeight-2*row_num)*radio);
            var left=parseFloat((parseInt(pos.x)/col_num)*width+x);    //设置相对浏览器窗口的偏移
            tag.css('left',left-12);
            tag.css('top',top-38);
        }
        if( $j('#viewer').find('.selected_area').length!=0){
            var sw=parseInt($j('.selected_area').attr('wr')*width);//重新计算高度
            var sh=parseInt($j('.selected_area').attr('hr')*height);
            var s_r=parseFloat(radio/$j('.selected_area').attr('c_r'));    //获取canvas相对于初始的缩放比例
            var s_left=parseInt($j('.selected_area').attr('left_gap')*s_r)+x;
            var s_top=parseInt($j('.selected_area').attr('top_gap')*s_r)+y;//偏移为相对位移+绝对位移
            $j('.selected_area').css({
                width:sw,
                height:sh,
                left: s_left,
    ...
    getZoomRadio:function(){
        var initialWidth=7817;                              //原图片的大小
        //var initialHeight=7425;
        var width=parseInt($j('.canvas').css('width'));    
        // var height=parseInt($j('.canvas').css('height'));//图片当前的尺寸
        var radio=width/initialWidth;
        //var time=parseInt(initialWidth/width);
        $j('.canvas').attr('r',parseFloat(radio));          //把缩放比例存放在r属性里面
        //$j('#zoomRadio').text(time);        
    }
