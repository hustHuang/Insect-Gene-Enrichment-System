...
class GeneRelation {
    ...
    function getStrainScoreByStrainID($strain_id,$tcondition){
        $dbc = DBCxn::get_conn();
        if (is_null($dbc))
            return null;
        $temperature = $this->getTemperatureByConditionType($tcondition);
        if (is_null($temperature))
            return null;
        $query = "SELECT s.fitness_score FROM SC_strain_smf s WHERE s.strain_id = ".$strain_id." and s.temperature = ".$temperature;
        $result = mysqli_query($dbc, $query);
        if (!$result)
            return null;
        if ($row = mysqli_fetch_row($result)) {
    ...
    function getTemperatureByConditionType($tcondition){
        $dbc = DBCxn::get_conn();
        if (is_null($dbc))
            return null;
        $query = "SELECT c.temperature FROM condition_type c WHERE c.id = ".$tcondition ;
        $result = mysqli_query($dbc, $query);
        if (!$result)
            return null;
        if ($row = mysqli_fetch_row($result)) {
    ...
    function getWithInteractionData($gene, $type, $score, $p_value) {
        ...
        for ($i = 0; $i < count($strain_query); $i++) {
            $query = "SELECT i.query, i.array, i.score, i.p_value,i.dataset,i.tcondition FROM SC_gi_interaction i WHERE i.score " . ($type == 'n' ? " < " : " > ") . $score . " AND i.p_value < " . $p_value . " AND  i.query = " . $strain_query[$i] . "  ORDER BY i.score " . ($type == 'n' ? "ASC" : "DESC");
            // echo $query;
            $result = mysqli_query($dbc, $query);
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_row($result)) {
                    ...
                    $strain1_score = $this->getStrainScoreByStrainID($row[0],$row[5]);
                    $strain2_score = $this->getStrainScoreByStrainID($row[1],$row[5]);
        ...
        for ($j = 0; $j < count($strain_array); $j++) {
            $query = "SELECT i.query, i.array, i.score, i.p_value,i.dataset,i.tcondition FROM SC_gi_interaction i WHERE  i.score " . ($type == 'n' ? " < " : " > ") . $score . " AND i.p_value < " . $p_value . " AND  i.array = " . $strain_array[$j] . " ORDER BY i.score " . ($type == 'n' ? "ASC" : "DESC");
            $result = mysqli_query($dbc, $query);
            //var_dump($result);
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_row($result)) {
    ...
    function getWithInInteractionData($gene1, $gene2, $type, $score, $p_value) {
        ...
        for ($i = 0; $i < count($strain_id_gene1); $i++) {
            for ($j = 0; $j < count($strain_id_gene2); $j++) {
                ...
                $query = "SELECT i.score, i.p_value, i.dataset, i.tcondition FROM SC_gi_interaction i WHERE i.score " . ($type == 'n' ? " < " : " > ") . $score . " AND i.p_value < " . $p_value . " AND i.query = " . $strain_id_gene1[$i] . " AND i.array = " . $strain_id_gene2[$j] . "";
                $dbc = DBCxn::get_conn();
                if ($dbc == null)
                    return null;
                $result = mysqli_query($dbc, $query);
                if (!$result)
                    return null;
                if ($row = mysqli_fetch_row($result)) {
                    $strain1_score = $this->getStrainScoreByStrainID($strain_id_gene1,$row[3]);
                    $strain2_score = $this->getStrainScoreByStrainID($strain_id_gene2,$row[3]);   
                    return $this->getInteraction($gene1_data, $gene2_data, $strain1_name,$strain1_score, $strain2_name,$strain2_score, $row[0], $row[1], $row[2]);
                }
            }
        }
