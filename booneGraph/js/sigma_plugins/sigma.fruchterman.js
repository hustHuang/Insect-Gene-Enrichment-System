/**
 * Linkurious 2012, all rights reserved. Sébastien Heymann <seb@linkurio.us>,
 * Romain Yon <romain@linkurio.us>
 * 
 * Please use http://jsbeautifier.org/ and indent with 2 spaces.
 * 
 * Lib docs: http://twitter.github.com/bootstrap/ http://docs.jquery.com/
 * http://addyosmani.github.com/jquery-ui-bootstrap/ http://sigmajs.org/ + well
...
sigma.fruchtermanReingold.FruchtermanReingold = function(graph, fixedNodes) {
    ...
    this.atomicGo = function() {
        ...
        edges.forEach(function(e) {
            ...
            var yDist = nf.y - nt.y;
            var dist = Math.sqrt(xDist * xDist + yDist * yDist) + 0.05;
            // var dist = Math.sqrt(xDist * xDist + yDist * yDist) - nf.size -
            // nt.size;
        ...
        nodes.forEach(function(n) {
            ...
            if (-1 == jQuery.inArray(n.id + '', fixedNodes)) {
                var xDist = n.fr.dx;
                var yDist = n.fr.dy;
                var dist = Math.sqrt(xDist * xDist + yDist * yDist);
                if (dist > 0) {
    ...
    this.reInit = function() {
        self.nodes = graph.nodes.filter(function(n) {
            return !n.hidden;
        });
        
        self.edges = graph.edges.filter(function(e) {
...
sigma.publicPrototype.stopFruchtermanReingold = function() {
    this.FruchtermanReingold.stop = true;
