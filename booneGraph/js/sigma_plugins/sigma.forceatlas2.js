...
sigma.forceatlas2.ForceAtlas2 = function(graph) {
  ...
  this.p = {
    ...
    edges: this.graph.edges.filter(function (e) { return !e.source.hidden && !e.target.hidden; })
  };
  ...
  this.atomicGo = function() {
    ...
    var nodes = self.p.nodes;
    var edges = self.p.edges;
  ...
  this.setAutoSettings = function() {
    ...
    // Performance
    if (self.p.nodes.length >= 50000) {
      this.p.jitterTolerance = 10;
    } else if (self.p.nodes.length >= 5000) {
      this.p.jitterTolerance = 1;
    } else {
      this.p.jitterTolerance = 0.1;
    }
    if (self.p.nodes.length >= 1000) {
...
//        var yDist = n.fa2.y() - r.getMassCenterY();
        ...
        if (distance > 0) {
          ...
          var factor = -this.coefficient * n.fa2.mass * r.config('mass') / distance;
